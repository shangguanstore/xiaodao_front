...
Page({
    ...
    onShow: function () {
        ...
        console.log('goodsList', shopList)
        let allGoodsPrice = 0
        let totalScoreToPay = 0
        shopList.map(item => {
            allGoodsPrice += item.price_format * item.number
            totalScoreToPay += item.point_format * item.number
        })
        that.setData({
            goodsList: shopList,
            allGoodsPrice,
    ...
    createOrder: function (e) {
        ...
        wx.request({
            ...
            success: (res) => {
                ...
                mallUtil.sendTempleMsg(res.data.data.id, -1,
                ...
                mallUtil.sendTempleMsg(res.data.data.id, 2,
                ...
                // 下单成功，跳转到订单管理界面
                wx.redirectTo({
                    url: "/mall/order-list/order-list"
                });
            }
        })
    },
    ...
    addAddress: function () {
        wx.navigateTo({
            url: "/mall/address-add/address-add"
        })
    },
    selectAddress: function () {
        wx.navigateTo({
            url: "/mall/select-address/select-address"
        })
    },
