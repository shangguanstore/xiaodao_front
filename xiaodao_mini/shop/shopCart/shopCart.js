...
Page({
    ...
    totalPrice: function () {
        ...
        for (var i = 0; i < list.length; i++) {
            var curItem = list[i];
            if (curItem.active) {
                total += parseFloat(curItem.price_format) * curItem.number;
                totalScoreToPay += curItem.point_format * curItem.number;
            }
        }
            ...
            if (!carShopBean.propertyChildIds || carShopBean.propertyChildIds == "") {
                ...
                request(url, 'post', {id: carShopBean.goodsId, cid: app.config.cid}, res=>{
                    ...
                    if (goods.properties) {
                        wx.showModal({
                            title: '提示',
                            content: goods.name + ' 商品已失效，请重新购买',
                            showCancel: false
                        })
                        isFail = true;
                        wx.hideLoading();
                    ...
                    if (goods.inventory < carShopBean.number) {
                        wx.showModal({
                            title: '提示',
                            content: goods.name + ' 库存不足，请重新购买',
                            showCancel: false
                        })
                        isFail = true;
                        wx.hideLoading();
                    ...
                    if (goods.price_format != carShopBean.price_format) {
                        wx.showModal({
                            title: '提示',
                            content: goods.name + ' 价格有调整，请重新购买',
                            showCancel: false
                        })
                        isFail = true;
                        wx.hideLoading();
                    ...
                    if (goods.point_format != carShopBean.point_format) {
                        wx.showModal({
                            title: '提示',
                            content: goods.name + ' 价格有调整，请重新购买',
                            showCancel: false
    ...
    navigateToPayOrder: function () {
        wx.hideLoading();
        wx.navigateTo({
            url: "/shop/toPayOrder/toPayOrder"
        })
    }
})